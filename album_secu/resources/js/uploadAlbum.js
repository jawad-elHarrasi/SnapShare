document.addEventListener("DOMContentLoaded", function () {
    ...
    document.querySelector("form").addEventListener("submit", async function (event) {
        ...
        const textElement = document.getElementById("albumDescription");
        const textData = new TextEncoder().encode(textElement.value);
        const encryptedText = await crypto.subtle.encrypt(
            { name: "RSA-OAEP" },
            publicKey,
            textData
        );
        document.getElementById("encryptedText").value = btoa(String.fromCharCode(...new Uint8Array(encryptedText)));
